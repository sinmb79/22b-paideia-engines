...
- ReviewLabel trust fields are validated at initialization: score must be an integer between 0 and 100, status와 reviewed_by는 surrounding whitespace가 없는 non-empty string이어야 하며, notes는 string이어야 합니다.
