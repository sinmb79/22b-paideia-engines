...
| 오케스트레이션 | 조합 | 각 엔진 내부 정책 |

## 설계 규칙

어떤 엔진도 다른 엔진의 결정을 조용히 대신하면 안 됩니다. 스트레스 엔진은 승급 후보 신호를 만들 수 있지만, 실제 승급 결정은 승급 엔진만 만들 수 있습니다. 런타임은 증거를 기록할 수 있지만 기억을 활성화하지 않습니다. 거버넌스는 실행을 허용하거나 차단할 수 있지만 모델 출력을 생성하지 않습니다.
