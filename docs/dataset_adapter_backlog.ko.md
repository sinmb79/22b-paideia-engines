...
- 공개 안전 acquired-source manifest 예제: `examples/acquired_sources_manifest.jsonl`
- JSONL parsing, acquired-source schema, duplicate record, content-scope, hash/license-note, auto-download, public-release safety diagnostics

Phase 11 diagnostics layer는 더 큰 로컬 교과서, 시험지, AI-Hub corpus를 엔진에 연결하기 전에 acquired-source manifest를 검증합니다. manifest record와 로컬 evidence만 읽으며, source content를 다운로드, scraping, 복사, 업로드하지 않습니다.
...
- 공개 안전 과목별 stress pack: `examples/stress_packs/core_subject_stress_pack.json`

Phase 12 stress pack은 candidate-only promotion boundary를 유지하면서 수학, 국어/언어, 과학, evidence-review scenario를 재사용 가능하게 추가합니다.
...
4. **Benchmark and evaluation fixture**
   - 입력: 각 엔진용 공개 안전 expected-output fixture
   - 출력: engine contract와 release-readiness regression check
   - 보호장치: 명시적 source right가 없는 한 fixture는 synthetic 또는 metadata-only로 유지
