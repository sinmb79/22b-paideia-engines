...
def engine_contracts() -> list[EngineContract]:
    ...
    return [
        ...
        EngineContract(
            name="curriculum",
            package="paideia_engines.curriculum",
            version="0.2",
            status="phase20_closed_loop_curriculum",
            public_api=[
                "WeaknessRecord",
                "CurriculumPlan",
                "AdaptiveExam",
            ...
            output_schemas=[
                ...
                "paideia-weakness-detection-report/v1",
                "paideia-curriculum-generation-report/v1",
                "paideia-adaptive-exam-generation-report/v1",
                "paideia-curriculum-completion/v1",
            ],
            cli_commands=[],
            examples=[],
            docs=[
                "src/paideia_engines/curriculum/README.md",
                "src/paideia_engines/curriculum/README.ko.md",
            ],
            safety_boundaries=[
                "failure memories are reviewable evidence, not hidden reasoning",
                "curriculum generation is deterministic and local-first",
                "high-severity weaknesses block strong reuse until remediation",
                "exam completion reduces weakness only through explicit pass evidence",
            ],
            compatibility="Weakness and curriculum records are additive before 1.0; remediation gates must fail closed.",
        ),
        EngineContract(
            name="cultivation",
